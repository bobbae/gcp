...
def poll_notifications(project, subscription_name, table_id):
    ...
    def callback(message):
        ...
        table_insert_rows(table_id, message)
        message.ack()
...
# this part from ../bigquery/test_bigquery_insert_rows.py
from google.cloud import bigquery


def table_insert_rows(table_id, message):
    ...
    attributes = message.attributes
    rows_to_insert = [
        {u"field1": attributes["eventType"], u"field2": attributes["bucketId"]},
        {u"field1": attributes["objectId"], u"field2": attributes["objectGeneration"]},
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "table_id", help="table ID"
    )
    args = parser.parse_args()
    poll_notifications(args.project, args.subscription, args.table_id)
